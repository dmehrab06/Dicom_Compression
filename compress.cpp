    ...
    for(int i = 0;i<r;++i){
        ...
        for(int j = 0;j<c;++j){
            ...
            while(cur->next){
                ...
                frmxbit = max(frmxbit,bits_needed[fr]);
                pxmxbit = max(pxmxbit,bits_needed[px]);

                bit_frequency_frame[bits_needed[fr]]++;
                bit_frequency_pixel[bits_needed[px]]++;
                
                if(fr==0)continue;
                
                out_contents.push_back(fr);
                out_contents.push_back(px);
            }
        }
    }

    printf("Maxmimum bit needed to represent a frame: %d\n",frmxbit);
    printf("Maxmimum bit needed to represent a pixel: %d\n",pxmxbit);

    printf("bit histogram for frames:\n");
    for(int bit = 1; bit<=frmxbit; ++bit)
    	printf("frequency of %d bit frames: %d\n",bit,bit_frequency_frame[bit]);

    printf("bit histogram for pixels:\n");
    for(int bit = 1; bit<=pxmxbit; ++bit)
    	printf("frequency of %d bit pixels: %d\n",bit,bit_frequency_pixel[bit]);
